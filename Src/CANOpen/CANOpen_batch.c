...
int8_t CAN_Check_Convergence(Control_Mode controlmode, uint8_t Node, int32_t Desired_value, float TensionError, CO_MOTOR *MotorStruct)
{
  ...
  {
    CANOpen_readPDO(Node, 1, &MotorStruct->TPDO[0], 2);
...
void CAN_Send_DesiredValue(Control_Mode controlmode, uint8_t Node, int32_t Desired_value, CO_MOTOR *MotorStruct)
{
  if (controlmode == Cyclic_sync_pos) // Position
  {
    Target_Pos[Node - 1] = Desired_value;
    if (Target_Pos[Node - 1] > Pos_ubound[Node - 1])
    {
      Target_Pos[Node - 1] = Pos_ubound[Node - 1];
    }
    else if (Target_Pos[Node - 1] < Pos_lbound[Node - 1])
    {
  ...
  else if (controlmode == Cyclic_sync_vel) // Velocity
  {
    ...
    else if (Target_Vel[Node - 1] < 0 && Pos_check[Node - 1] < Pos_lbound[Node - 1])
    {
      Target_Vel[Node - 1] = 0;
    }
  ...
  void CAN_Manual_Control(Control_Mode Con_Mode, uint8_t Node, int32_t Desired_input_PV, uint16_t Desired_input_A, int16_t Desired_input_F)
  {
  }
